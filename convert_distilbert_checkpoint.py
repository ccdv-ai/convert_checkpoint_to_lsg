...
def update_positions(model, max_pos):
    position_embeddings_weights = model.distilbert.embeddings.position_embeddings.weight.clone()
    ...
    model.distilbert.embeddings.position_ids = torch.arange(max_pos, device=model.distilbert.embeddings.position_ids.device).unsqueeze(0)
    model.distilbert.embeddings.position_embeddings.weight.data = new_position_embeddings_weights
    return model
...
def update_global(model, bos, mask, stride, keep_first=False):
    ...
    u = model.distilbert.embeddings.word_embeddings.weight.clone()
    positions = model.distilbert.embeddings.position_embeddings.weight.clone()
    ...
    if keep_first:
        model.distilbert.embeddings.global_embeddings.weight.data[1:] = positions[1:]
    else:
        model.distilbert.embeddings.global_embeddings.weight.data = positions
    return model
...
def update_global_randomly(model, bos, stride, keep_first=False):
    ...
    if keep_first:
        model.distilbert.embeddings.global_embeddings.weight.data[1:] = (w + positions)[1:]
    else:
        model.distilbert.embeddings.global_embeddings.weight.data = w + positions
    return model
...
def main():
    ...
    if model_types is None:
        model_types = ["DistilBertForMaskedLM"]
    ...
    # Load model
    config = LSGDistilBertConfig.from_pretrained(args.initial_model, architectures=_architectures)
