...
class DataTrainingArguments:
    ...
    task_name: Optional[str] = field(
        default='ecthr_a',
        metadata={
            "help": "Define downstream task_name"
        },
    )
...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset(dataset_path, name=data_args.task_name, split="train", data_dir='data', cache_dir=model_args.cache_dir)
    ...
    if training_args.do_eval:
        split = "test" if model_args.eval_with_test else "validation"
        eval_dataset = load_dataset(dataset_path, name=data_args.task_name, split=split, data_dir='data', cache_dir=model_args.cache_dir)

    if training_args.do_predict:
        predict_dataset = load_dataset(dataset_path, name=data_args.task_name, split="test", data_dir='data', cache_dir=model_args.cache_dir)
    ...
    config = AutoConfig.from_pretrained(
        ...
        finetuning_task_name=f"{data_args.task_name}",
